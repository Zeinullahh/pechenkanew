...
**Key Characteristics:**
- True black (`#000000`) dominant surfaces with white and blue as the only relief colors
- LamboType custom Neo-Grotesk font with 12° angled terminals inspired by aerodynamic car lines
- Silence Blue (`#3B82F6`) as the sole accent color — used exclusively for primary CTA buttons
...
- Rounded pill buttons and frosted-glass cards — see the Glass Update addendum (§10), which supersedes the zero border-radius rules below
...
### Primary
- **Silence Blue** (`#3B82F6`): The signature accent color — a vivid, saturated blue (rgb 59, 130, 246) used exclusively for primary action buttons ("Discover More", "Tickets", "Start Configuration"). The only chromatic color in the entire interface, it ignites against the black canvas like a headlight cutting through night
...
### Secondary & Accent
- **Deep Blue** (`#1D4ED8`): Hover/pressed state for blue buttons — a deep royal blue (rgb 29, 78, 216) that darkens the accent to signal interaction
- **Blue Text** (`#60A5FA`): Slightly lighter blue variant (rgb 96, 165, 250) used for inline text accents, star ratings, and highlighted labels
...
| Button Large | 16px (1.00rem) | 400 | 1.50 | normal | Blue CTA buttons |
...
### Buttons
Primary CTA buttons use a **rounded pill shape** (`border-radius: 999px`) — the glass-era update (§10) supersedes the original zero-radius rule.

**Blue Accent CTA** — The primary action:
- Default: bg `#3B82F6` (Silence Blue), text `#FFFFFF`, padding 24px, fontSize 16px, fontWeight 400, rounded pill (`border-radius: 999px`), no border
- Hover: bg `#1D4ED8` (Deep Blue), darkens significantly
...
- Used for: CTAs on dark sections where blue isn't appropriate
...
- **Button padding**: 16px (ghost), 24px (blue accent)
...
- Apply Silence Blue (`#3B82F6`) exclusively for primary CTA buttons — never for decorative purposes
...
### Don't
- Introduce additional accent colors beyond blue — the monochrome-plus-blue system is sacred
- Use yellow, gold, or amber anywhere in the interface — the former gold accent has been fully replaced by Silence Blue (`#3B82F6`)
...
### Touch Targets
- Blue CTA buttons: 48px+ minimum height with 24px padding (exceeds WCAG 44×44px)
...
- Primary CTA: "Silence Blue (#3B82F6)"
...
- "Create a hero section with a full-viewport black background, the model name 'TEMERARIO' in LamboType at 120px uppercase weight 400 white text with 0.92 line-height, centered vertically, with a Silence Blue (#3B82F6) 'Discover More' button below — rounded pill, 24px padding, white text"
...
3. Use natural language descriptions, not CSS values — "vivid blue rounded pill" not "border-radius: 999px; background: #3B82F6"
...
## 10. Addendum — 2026-08 Glass Update

The Lamborghini-inspired system above remains the base, but the following site-wide updates now apply (superseding the "no glows/blur on UI elements" rule from §6):
...
### 2026-08-24 — Blue Accent & Rounded Glass Update

- **Blue replaces gold**: the former Lamborghini Gold (`#FFC000`) accent is fully retired. All primary CTAs, accent dots, badges, star ratings, and highlights now use **Silence Blue (`#3B82F6`)** with **Deep Blue (`#1D4ED8`)** hover/pressed states and **Blue Text (`#60A5FA`)** for inline accents. No yellow, gold, or amber may appear anywhere in the interface.
- **Rounded glass everywhere**: every card, panel, modal, banner, dropdown, tooltip, and floating control uses the `.glass-readable` recipe (or an equivalent translucent background + `backdrop-blur`) with generous corner radii — typically `24–32px` for cards and panels, `rounded-full` for pills, chips, icon buttons, and CTA buttons. Sharp (`0px`) corners are no longer allowed on visible UI surfaces.
- **Rounded CTAs**: primary buttons are blue pills (`rounded-full`) with white text, matching the floating glass header pill.
- **Floating dock**: the social dock is a dark frosted-glass pill (`bg-white/10`, `backdrop-blur-xl`, `rounded-full`) with circular icons — the former opaque beige bar is retired.
- **EdgeGlowCard**: the glow frame defaults to a `26px` border radius so the hover glow follows the rounded card corners; pass `borderRadius` to match the outer card radius when it differs.
